...
lemma correlation_sum_approx_N {N : ℕ} [NeZero N]
  ...
  convert mul_le_mul_of_nonneg_right ( P.correlation_bound h ) ( Nat.cast_nonneg N ) using 1 ; ring_nf;
  convert abs_mul ( N : ℝ ) ( -1 + ( ∑ x : Fin N, P.nu x * P.nu ( x + h ) ) * ( N : ℝ ) ⁻¹ ) using 1 ; ring_nf;
...
lemma mass_approx_N {N : ℕ} [NeZero N]
  ...
  rw [ div_sub_one, abs_div, abs_of_nonneg ( by positivity : ( 0 : ℝ ) ≤ N ), div_mul_cancel₀ _ ( by norm_cast; exact NeZero.ne N ) ] ; ring_nf;
...
lemma additiveEnergy_eq_sum_correlationSq {N : ℕ} [NeZero N] (f : Fin N → ℝ) :
  ...
  unfold additiveEnergy' correlationSum; ring_nf;
