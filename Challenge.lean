...
def selbergFourier (N : ℕ) [NeZero N] (lambda : ℕ → ℝ) (P : ℕ) : Fin N → ℂ :=
...
open Lean Meta in
run_cmd modifyEnv fun env => auxLemmasExt.setState env {}
...
def sieveMass (N : ℕ) (lambda : ℕ → ℝ) (P : ℕ) : ℝ :=
...
/-- **multiPrimeQuadForm_perturbation**
    If λ, μ are bounded by C and differ by at most δ_d pointwise,
    then |Q(λ) − Q(μ)| ≤ 2C · Σ_{d,e} δ_d / lcm(d,e). -/
theorem multiPrimeQuadForm_perturbation
    (P : ℕ) (hP : Squarefree P)
    (lambda mu : ℕ → ℝ)
    (C : ℝ) (hC : 0 ≤ C)
    (delta : ℕ → ℝ) (hdelta : ∀ d, 0 ≤ delta d)
...
def correlationBound (P m : ℕ) (lambda : ℕ → ℝ) (h : Fin (P * m)) : ℝ :=
  ∑ d ∈ sqfDivisors P, ∑ e ∈ sqfDivisors P,
    |lambda d| * |lambda e| / (Nat.lcm d e : ℝ)
...
lemma correlationBound_nonneg (P m : ℕ) (lambda : ℕ → ℝ) (h : Fin (P * m)) :
    0 ≤ correlationBound P m lambda h := by
  sorry
